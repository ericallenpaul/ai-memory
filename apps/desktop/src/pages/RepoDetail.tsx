...
export function RepoDetail() {
  ...
  // Load symbol outline when a file is selected.
  useEffect(() => {
    if (!selectedFile || status !== "ready") { setSymbols([]); return; }
    (async () => {
      try {
  ...
  // Load symbol source when a symbol is selected.
  useEffect(() => {
    if (!selectedSymbol || status !== "ready") { setSymbolSource(null); return; }
    (async () => {
      try {
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 2fr", gap: 12, height: "calc(100vh - 140px)" }}>

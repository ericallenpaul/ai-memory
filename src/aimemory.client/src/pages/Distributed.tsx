 ...
 * with a revoke button each. See design doc §4.1 / §4.3.
 */
export default function Distributed() {
  ...
  const refresh = async () => {
    try {
      const s = await distributedStatus(api);
      setStatus(s);
      setStatusError(null);
    } catch (e) {
    ...
    try {
      const p = await pairingsList(api);
      setPairings(p);
  ...
  const openEnableDialog = async () => {
    try {
      const list = await listNetworkInterfaces(api);
      setInterfaces(list);
  ...
  const handleEnable = async () => {
    ...
    setBusy("enable");
    try {
      const resp = await distributedEnable(api, iface || undefined);
      setRevealed(resp);
  ...
  const handleDisable = async () => {
    ...
    setBusy("disable");
    try {
      const resp = await distributedDisable(api);
  ...
  const handleRestartNow = async () => {
    setBusy("restart");
    try {
      await serviceCommand(api, "aimemory-api", "restart");
  ...
  const handleRevoke = async (p: PairingRow) => {
    ...
    try {
      await pairingsRevoke(api, p.pairingId);
      await refresh();
    } catch (e) {
